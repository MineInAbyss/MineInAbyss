...
class StarCompassSystem : TickingSystem(interval = 0.1.seconds) {
    ...
    override fun TargetScope.tick() {
        ...
        entity.has<PlayerCompassBar>().broadcastVal()
        var bar = compassBar
        if (entity.has<PlayerCompassBar>()) {
            entity.encodeComponentsTo(player)
            bar = entity.get<PlayerCompassBar>()?.compassBar!!
        }
        else {
            entity.setPersisting(PlayerCompassBar)
        }
        ...
        if (!gearyCompass.has<HideBossBarCompass>()) {
            ...
            player.bossbarCompass(starCompass.compassLocation!!, bar)
        }
        else {
