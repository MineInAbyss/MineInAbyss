...
class PatreonConfig(
    ...
    val patreonRoles: Map<String, PatreonRoles> = emptyMap(),
) {
    @Serializable
...
val PatreonFeature = module("patreon") {
    val config = get<AbyssFeatureConfig>().patreon
    requirePlugins("LuckPerms", "DiscordSRV")

    single { PatreonModule() }
    val listener = PatreonListener(config)

    listeners(listener)
    DiscordSRV.api.subscribe(listener)
    addCloseable { DiscordSRV.api.unsubscribe(listener) }
}.mainCommand {
    "patreon" {
        ...
        "prefix" {
            ...
            "set" {
                executes.asPlayer().args(
                    ...
                    "location" to Args.string().oneOf { get<PatreonModule>().prefixContexts }.default { "global" }
                ) { emote, loc ->
                    val player = sender as Player
