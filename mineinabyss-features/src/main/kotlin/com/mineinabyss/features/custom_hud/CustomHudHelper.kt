...
val LAYOUT_KEY = NamespacedKey.fromString("hud_layouts", mythicHud)!!
val Player.activeLayouts get() = persistentDataContainer.get(LAYOUT_KEY, PersistentDataType.TAG_CONTAINER)?.keys?.mapNotNull { mythicHud.layouts().get(it.key) } ?: emptyList()
...
fun toggleBackgroundLayouts(player: Player, feature: CustomHudFeature) {
    ...
    val oldActiveLayouts = player.activeLayouts.minus(backgroundLayout)
